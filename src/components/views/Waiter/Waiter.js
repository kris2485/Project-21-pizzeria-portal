...
class Waiter extends React.Component {
  ...
  renderActions(tableId, status) {
    ...
    switch (status) {
      ...
      case 'thinking':
        return <Button onClick={() => fetchStatus(tableId, 'new order')}>new order</Button>;
      case 'ordered':
        return <Button onClick={() => fetchStatus(tableId, 'prepared')}>prepared</Button>;
      case 'prepared':
        return <Button onClick={() => fetchStatus(tableId, 'delivered')}>delivered</Button>;
      case 'delivered':
        return <Button onClick={() => fetchStatus(tableId, 'paid')}>paid</Button>;
      case 'paid':
        return <Button onClick={() => fetchStatus(tableId, 'free')}>free</Button>;
      default:
        return null;
    }
  }
